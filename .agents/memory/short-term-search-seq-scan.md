...
**Fix applied:** warmers skip prefixes <3 chars; buildBeerSearchFragments has a short-query branch (all tokens <3 chars) that switches to PREFIX autocomplete semantics (`LIKE 'xx%'`) served by btree text_pattern_ops index `idx_beers_name_unaccent_prefix` (created in the startup ensure-indexes block; must be created as the app user — superuser lacks unaccent in search_path). Multi-token short queries keep per-token AND prefix semantics.
