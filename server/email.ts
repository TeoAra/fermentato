...
export async function sendWishlistBeerAvailableEmail(
  toEmail: string,
  beerName: string,
  pubName: string,
  pubCity: string,
  ...
  const pubUrl = `${APP_URL}/pub/${pubSlug}`;
  const subject = `🍺 "${escapeHtml(beerName)}" è disponibile vicino a te`;
  const html = `<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Birra wishlist disponibile — Fermenta.to</title>
</head>
<body style="margin:0;padding:0;background:#fafaf8;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#fafaf8;padding:40px 20px;">
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:40px;">
              <h2 style="color:#1f2937;font-size:22px;font-weight:600;margin:0 0 16px;">Una birra dalla tua wishlist è disponibile!</h2>
              <p style="color:#4b5563;font-size:16px;line-height:1.6;margin:0 0 8px;">
                <strong>${escapeHtml(beerName)}</strong> è ora disponibile ${sourceLabel} da
              </p>
              <p style="color:#1f2937;font-size:18px;font-weight:600;margin:0 0 4px;">${escapeHtml(pubName)}</p>
              <p style="color:#6b7280;font-size:14px;margin:0 0 28px;">${escapeHtml(pubCity)}</p>
              <div style="text-align:center;margin:32px 0;">
                <a href="${pubUrl}" style="display:inline-block;background:linear-gradient(135deg,#f59e0b,#ea580c);color:#ffffff;font-size:16px;font-weight:600;text-decoration:none;padding:14px 36px;border-radius:10px;">
                  Vedi il locale
                </a>
              </div>
              <hr style="border:none;border-top:1px solid #e5e7eb;margin:28px 0;">
  ...
  if (!transport) {
    console.log(`[email] wishlist-beer email (no SMTP): to=${toEmail} subject="${subject}"`);
    return;
  }
  try {
    await transport.sendMail({ from: FROM_ADDRESS, to: toEmail, subject, html });
    console.log(`[email] wishlist-beer email sent to ${toEmail}`);
  } catch (err: any) {
    console.error(`[email] wishlist-beer email failed for ${toEmail}:`, err.message);
  }
}
