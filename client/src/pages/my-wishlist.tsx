      ...
      {isLoading ? (
        ...
        <div className="p-4 space-y-2">
          {wishlist.map((item: any) => (
            <div
              key={item.beer_id}
              className="bg-white dark:bg-[hsl(220,5%,18%)] rounded-2xl shadow-sm flex items-center gap-3 p-3"
            >
              {item.beer_image ? (
              ...
              <Link href={`/beer/${item.beer_id}`} className="flex-1 min-w-0">
                ...
                {item.beer_style && (
                  <span className="inline-block text-xs bg-stone-100 dark:bg-[hsl(220,5%,22%)] text-stone-500 rounded-full px-2 py-0.5 mt-1">{item.beer_style}</span>
                )}
              </Link>
              {item.beer_abv && (
