  ...
  return (
    ...
    >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        {/* Festival Mode CTA */}
        <Link href="/festival">
          <div className={`bg-white dark:bg-card border border-primary/20 dark:border-primary/20 bg-gradient-to-r from-orange-50 to-white dark:from-orange-950/10 dark:to-card rounded-2xl p-5 mb-4 flex items-center justify-between gap-4 cursor-pointer hover:shadow-md transition-all group ${activeTab !== 'overview' ? 'hidden lg:flex' : ''}`}>
        ...
        <Link href={`/microblog/nuovo?breweryId=${brewery?.id}&breweryName=${encodeURIComponent(brewery?.name || '')}&authorType=brewery&authorEntityId=${brewery?.id}`}>
          <div className={`bg-white dark:bg-card border border-blue-100 dark:border-blue-900/30 bg-gradient-to-r from-blue-50 to-white dark:from-blue-950/10 dark:to-card rounded-2xl p-5 mb-8 flex items-center justify-between gap-4 cursor-pointer hover:shadow-md transition-all group ${activeTab !== 'overview' ? 'hidden lg:flex' : ''}`}>
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0 text-left">
                <p className="font-bold text-foreground">Pubblica aggiornamento</p>
                <p className="text-sm text-muted-foreground truncate">Condividi novità, uscite o eventi con la community</p>
              </div>
            </div>
            <ExternalLink className="h-5 w-5 text-blue-500 shrink-0 opacity-50 group-hover:opacity-100 transition-opacity" />
          </div>
        </Link>
              ...
              <div className="space-y-2">
                ...
                <Link href={`/microblog/nuovo?breweryId=${brewery?.id}&breweryName=${encodeURIComponent(brewery?.name || '')}&authorType=brewery&authorEntityId=${brewery?.id}`}>
                  <div className="w-full flex items-center gap-3 p-3 rounded-2xl bg-blue-50/70 dark:bg-blue-950/20 backdrop-blur-xl border border-blue-100 dark:border-blue-900/30 shadow-[0_4px_20px_rgba(0,0,0,0.04)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] tap-scale active:scale-[0.99] transition-all text-left">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-foreground leading-tight">Pubblica aggiornamento</div>
                      <div className="text-[11px] text-muted-foreground truncate mt-0.5">Scrivi un post sulla community</div>
                    </div>
                    <ChevronRight className="h-4 w-4 text-stone-400 flex-shrink-0" />
                  </div>
                </Link>
