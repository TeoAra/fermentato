  ...
  return (
    <div className="min-h-screen bg-background">
                ...
                <div key={r.id} className="bg-white/70 dark:bg-white/[0.04] backdrop-blur-xl rounded-2xl border border-white/40 dark:border-white/[0.06] shadow-[0_4px_20px_rgba(0,0,0,0.04)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-200">
                  <div className="p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border border-stone-100 dark:border-[#23262E]/50 ${r.type === 'beer' ? 'bg-stone-50 dark:bg-[#0B0D10]/20' : 'bg-blue-50 dark:bg-blue-900/20'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge className={`text-xs rounded-full font-medium ${r.type === 'beer' ? 'bg-stone-50 text-primary border-stone-200 dark:bg-[#0B0D10]/40 dark:text-orange-400' : 'bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/40 dark:text-blue-400'}`} variant="outline">
                          ...
                          {r.status === 'pending' && <Badge className="bg-stone-50 text-primary dark:bg-[#0B0D10]/20 dark:text-orange-400 text-xs rounded-full font-medium border-stone-200 dark:border-[#23262E]/50">In attesa</Badge>}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {r.style && <span className="text-xs bg-stone-50/50 dark:bg-[#0B0D10]/10 text-primary dark:text-orange-400 px-2.5 py-0.5 rounded-full font-medium border border-stone-100 dark:border-[#23262E]/30">{r.style}</span>}
                          {r.abv && <span className="text-xs bg-stone-50/50 dark:bg-[#0B0D10]/10 text-primary dark:text-orange-400 px-2.5 py-0.5 rounded-full font-medium border border-stone-100 dark:border-[#23262E]/30">{r.abv}%</span>}
                          {r.city && <span className="text-xs bg-stone-50/50 dark:bg-[#0B0D10]/10 text-primary dark:text-orange-400 px-2.5 py-0.5 rounded-full font-medium border border-stone-100 dark:border-[#23262E]/30">{r.city}</span>}
                          {r.country && r.country !== 'Italia' && <span className="text-xs bg-stone-50/50 dark:bg-[#0B0D10]/10 text-primary dark:text-orange-400 px-2.5 py-0.5 rounded-full font-medium border border-stone-100 dark:border-[#23262E]/30">{r.country}</span>}
                    ...
                    {/* Expanded details */}
                    {isExpanded && (
                      <div className="mt-3 pl-13 space-y-2 text-sm border-t border-stone-100 dark:border-border pt-3">
                        ...
                        {r.adminNotes && (
                          <div className="bg-stone-50/30 dark:bg-[#0B0D10]/10 p-3 rounded-xl border border-stone-100 dark:border-[#23262E]/30">
                    ...
                    {r.status === 'pending' && (
                      <div className="flex gap-2 mt-3 pt-3 border-t border-stone-100 dark:border-border">
                        <Button
                          size="sm"
                        ...
                        <Button
                          ...
                          className="flex-1 border-stone-200 dark:border-border text-destructive hover:bg-stone-50 rounded-xl font-semibold gap-1.5"
                        >
      ...
      <AlertDialog open={approveId !== null} onOpenChange={open => !open && setApproveId(null)}>
        <AlertDialogContent className="rounded-2xl border-stone-200 dark:border-border">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="rounded-xl border-stone-200 dark:border-border hover:bg-stone-50">Annulla</AlertDialogCancel>
            <AlertDialogAction
