/**
 * PubQuickStats — compact stats widget for the pub owner dashboard overview.
 *
 * Shows at a glance:
 *  • Profile views last 7 days, with ↑↓ trend vs the previous 7-day window
 *  • Check-ins this month
 *  • Favorites count
 *  • Top beer (most checked-in at this pub)
 *
 ...
 *  - GET /api/pubs/:id/analytics      (page-view daily series)
 *  - GET /api/pubs/:id/stats-extended (checkins, favorites, top beer, checkinSeries)
 */
...
export default function PubQuickStats({ pubId }: PubQuickStatsProps) {
  ...
  const { data: analytics } = useQuery<{
    ...
    enabled: !!pubId,
    staleTime: 24 * 60 * 60 * 1000, // 24h — matches "dati aggiornati ogni 24h"
  });

  /* extended stats: favorites, checkins, top beers, checkinSeries */
  const { data: stats } = useQuery<{
    favorites: number;
    checkinsMonth: number;
    checkinsTotal: number;
    topBeersAllTime: { id: number; name: string; brewery: string; checkins: number; avgRating: number; imageUrl?: string }[];
  ...
  const last7Views = analytics?.last7 ?? 0;
  const prev7Views = analytics?.series
    ? analytics.series.slice(16, 23).reduce((s, d) => s + d.views, 0)
    : 0;
  const viewsTrendPct =
    prev7Views > 0
  ...
  const viewValues = (analytics?.series ?? []).map((d) => d.views);
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div className="bg-stone-50 dark:bg-[#12151A] rounded-xl p-3 space-y-1.5">
              ...
              <div className="flex items-end justify-between gap-2">
                <span className="text-2xl font-black text-foreground leading-none">
                  {last7Views.toLocaleString("it-IT")}
                </span>
              ...
              <p className="text-[10px] text-muted-foreground">
                Tot. 30 giorni: {(analytics?.last30 ?? 0).toLocaleString("it-IT")}
              </p>
            </div>
            ...
            <div className="bg-stone-50 dark:bg-[#12151A] rounded-xl p-3 space-y-1.5">
              ...
              <p className="text-[10px] text-muted-foreground">
                Utenti che hanno salvato il pub
              </p>
            </div>
        ...
        <div className="lg:hidden px-4 pb-3">
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              <strong className="text-foreground">{last7Views}</strong> visite
            </span>
            <span className="flex items-center gap-1">
