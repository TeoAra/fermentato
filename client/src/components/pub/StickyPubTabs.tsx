...
export default function StickyPubTabs({ tabs, activeTab, onTabChange }: StickyPubTabsProps) {
  ...
  const node = (
    ...
    >
      <div className="flex min-h-[52px] items-center justify-center px-1 gap-1">
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative min-h-12 flex flex-col items-center justify-center gap-0.5 px-3 min-w-[60px] rounded-[18px] transition-colors duration-200 active:scale-95 ${
                isActive
                  ? "text-[#F59E0B]"
