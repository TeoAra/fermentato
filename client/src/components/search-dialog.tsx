...
type TabId = "all" | "beers" | "breweries" | "pubs" | "users";

const TABS: { id: TabId; label: string; icon: string }[] = [
  { id: "beers",     label: "Birre",     icon: "🍺" },
  { id: "breweries", label: "Birrifici", icon: "🏭" },
  { id: "pubs",      label: "Locali",    icon: "🏠" },
...
export default function SearchDialog({ isOpen, onClose }: SearchDialogProps) {
  ...
  const [activeTab, setActiveTab] = useState<TabId>("beers");
  ...
  const handleResultClick = (type: string, id: number | string) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const all = [
      ...(activeTab === "beers"     || activeTab === "all" ? (searchResults.beers?.slice(0, beersShown) || []).map((i: any) => ({ ...i, _type: "beer" })) : []),
      ...(activeTab === "breweries" || activeTab === "all" ? (searchResults.breweries?.slice(0, breweriesShown) || []).map((i: any) => ({ ...i, _type: "brewery" })) : []),
      ...(activeTab === "pubs"      || activeTab === "all" ? (searchResults.pubs?.slice(0, pubsShown) || []).map((i: any) => ({ ...i, _type: "pub" })) : []),
    ];
    if (e.key === "ArrowDown") { e.preventDefault(); setSelectedIndex(p => Math.min(p + 1, all.length - 1)); }
      ...
      if (sel) { saveSearch(searchTerm); handleClose(); navigate(`/${sel._type}/${sel.id}`); }
  ...
  const counts: Record<TabId, number> = {
    all: (searchResults?.pubs?.length || 0) + (searchResults?.breweries?.length || 0) + (searchResults?.beers?.length || 0) + (searchResults?.users?.length || 0),
    beers:     searchResults?.beers?.length || 0,
    breweries: searchResults?.breweries?.length || 0,
    pubs:      searchResults?.pubs?.length || 0,
  ...
  const tabLabel: Record<TabId, string> = {
    all:       "risultati",
    beers:     tabCount === 1 ? "birra" : "birre",
    breweries: tabCount === 1 ? "birrificio" : "birrifici",
    pubs:      tabCount === 1 ? "locale" : "locali",
    users:     tabCount === 1 ? "utente" : "utenti",
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-4 pt-4 pb-3 border-b border-stone-100 dark:border-white/[0.06]">
          <div className="flex-1 flex items-center gap-2 bg-stone-100 dark:bg-[#1A1D24] rounded-full px-4 py-2.5">
            ...
            {!isLoading && searchTerm && (
              <button onClick={() => setSearchTerm("")} className="h-4.5 w-4.5 flex items-center justify-center text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 transition-colors flex-shrink-0">
        ...
        {/* ── Tabs ── */}
        <div className="flex items-center gap-1 px-3 py-2 border-b border-stone-100 dark:border-white/[0.06]">
          {TABS.map(tab => (
            <button
              key={tab.id}
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                activeTab === tab.id
                  ...
                  : "text-stone-500 dark:text-stone-400 hover:bg-stone-100 dark:hover:bg-[#1A1D24]"
              }`}
            >
              <span>{tab.icon}</span>
              {tab.label}
              {searchResults && counts[tab.id] > 0 && (
                <span className={`text-[10px] font-bold px-1 py-0.5 rounded-full min-w-[18px] text-center ${
                  activeTab === tab.id ? "bg-white/20" : "bg-stone-100 dark:bg-[#1A1D24] text-stone-500 dark:text-stone-400"
                }`}>
                  {counts[tab.id]}
                </span>
              )}
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              {recentSearches.length > 0 ? (
                <div>
                  ...
                  {recentSearches.map((s, i) => (
                    <button key={i} onClick={() => setSearchTerm(s)} className="flex items-center gap-3 w-full py-2.5 text-left group">
                      ...
                      <span role="button" onClick={e => removeSearch(s, e as any)} className="text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 transition-opacity p-1">
                        <X className="h-3.5 w-3.5" />
                      </span>
                    </button>
                  ))}
          ...
          {/* Results */}
          {!isLoading && searchResults && debouncedSearch.length >= 2 && (
            <div className="pb-4">
              {tabCount > 0 && (
                <p className="px-4 pt-3 pb-2 text-[11px] font-semibold uppercase tracking-wider text-stone-400 dark:text-stone-500">
                  {tabCount} {tabLabel[activeTab]}
                </p>
              )}

              {/* ── BIRRE ── */}
              {(activeTab === "beers") && searchResults.beers?.length > 0 && (() => {
                    ...
                    {visible.map((beer: any, i: number) => (
                      <button
                        ...
                        className={`flex items-center gap-3 w-full px-4 py-2.5 text-left transition-colors ${
                          selectedIndex === i ? "bg-stone-50 dark:bg-white/5" : "hover:bg-stone-50 dark:hover:bg-white/5"
                        }`}
                      >
                        <ImageWithFallback src={beer.imageUrl || beer.brewery?.logoUrl} alt={beer.name} imageType="beer"
              ...
              {/* ── BIRRIFICI ── */}
              {(activeTab === "breweries") && searchResults.breweries?.length > 0 && (() => {
              ...
              {/* ── LOCALI ── */}
              {(activeTab === "pubs") && searchResults.pubs?.length > 0 && (() => {
              ...
              {(activeTab === "users") && searchResults.users?.length > 0 && (
                <div>
                  {searchResults.users.map((u: any, i: number) => (
                    <button
                      key={`user-${u.id}`}
                      onClick={() => { saveSearch(searchTerm); handleClose(); navigate(`/user/${u.nickname || u.id}`); }}
                      className={`flex items-center gap-3 w-full px-4 py-2.5 text-left transition-colors ${
                        selectedIndex === i ? "bg-stone-50 dark:bg-white/5" : "hover:bg-stone-50 dark:hover:bg-white/5"
                      }`}
                    >
              ...
              {/* Nessun risultato nella tab attiva */}
              {tabCount === 0 && (
