...
export default function PubCard({ pub, beersOnTap = 0, isOpen = true }: PubCardProps) {
  return (
    <Link href={`/pub/${pub.id}`}>
      <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
        <div className="relative">
          <img
            src={pub.imageUrl || "https://images.unsplash.com/photo-1514933651103-005eec06c04b?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=240"}
            alt={`Interno ${pub.name}`}
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className={`flex items-center ${isOpen ? 'text-green-600' : 'text-red-600'}`}>
