...
function CommunityPostCard({ post }: { post: any }) {
  ...
  return (
    <div className="bg-white dark:bg-[hsl(220,5%,18%)] rounded-2xl border border-stone-100 dark:border-stone-800 shadow-sm p-4" data-testid={`community-post-${post.id}`}>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {post.pub_name && (
            <Link href={`/pub/${post.pub_id}`}>
              <span className="text-[10px] bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 px-2 py-0.5 rounded-full cursor-pointer hover:bg-stone-200 dark:hover:bg-stone-700">📍 {post.pub_name}</span>
            </Link>
          )}
          {post.brewery_name && (
            <Link href={`/brewery/${post.brewery_id}`}>
              <span className="text-[10px] bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 px-2 py-0.5 rounded-full cursor-pointer hover:bg-stone-200 dark:hover:bg-stone-700">🏭 {post.brewery_name}</span>
            </Link>
          )}
        </div>
      )}
  ...
  return (
    <section className="mt-8 space-y-3" data-testid={`community-posts-${entity.kind}-${entity.id}`}>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-[hsl(220,5%,18%)] rounded-2xl border border-dashed border-stone-200 dark:border-stone-700 p-6 text-center">
