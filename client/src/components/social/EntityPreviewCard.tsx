...
import { Star, ExternalLink, Loader2, MapPin, Building2 } from "lucide-react";
import { Button } from "@/components/ui/button";

export type EntityType = "brewery" | "pub";
...
export function EntityPreviewCard({ type, id, anchorRect, onClose }: EntityPreviewCardProps) {
  ...
  const endpoint = type === "brewery" ? `/api/breweries/${id}` : `/api/pubs/${id}`;
  ...
  const goToPage = () => {
    setLocation(type === "brewery" ? `/brewery/${id}` : `/pub/${id}`);
    onClose();
  };
