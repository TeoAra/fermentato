...
export function IosInstallGuide({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end justify-center p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-[#0B0D10] rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8"
        ...
        <div className="p-6">
          <div className="flex justify-center gap-2 mb-6">
            {steps.map((_, i) => (
              <div
                key={i}
                className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'w-8 bg-amber-500' : i < step ? 'w-4 bg-amber-300' : 'w-4 bg-stone-200 dark:bg-[#12151A]'}`}
              />
            ))}
          </div>

          <div className="text-center space-y-4">
            <div className="w-16 h-16 bg-stone-100 dark:bg-[#1A1D24] rounded-2xl flex items-center justify-center mx-auto">
              {current.icon}
            </div>
            <div>
  ...
  return (
    <>
      ...
      <div className="fixed bottom-nav-above left-4 right-4 lg:left-auto lg:right-4 lg:w-96 z-50 animate-in slide-in-from-bottom-5">
        <div className="bg-white/90 dark:bg-[#0B0D10]/90 backdrop-blur-xl border border-amber-200 dark:border-amber-800 rounded-2xl p-4 shadow-2xl">
...
export function CapacitorLocationPrompt() {
  ...
  return (
    <div className="fixed bottom-36 left-4 right-4 z-50">
      <div className="bg-white dark:bg-[#0B0D10] border border-amber-200 dark:border-amber-800 rounded-2xl p-4 shadow-2xl">
        <div className="flex items-start gap-3">
          <div className="p-2 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl flex-shrink-0">
