...
//
// IMPORTANTE: NON usa 'import GoogleCast'.
// Le classi GCK sono esposte via bridging header (App/App-Bridging-Header.h)
// che Xcode inietta automaticamente in tutti i file Swift del target App.
// Questo bypassa il bug Xcode 16 che impedisce di importare XCFramework binari
// pre-compilati come moduli Swift nativi.
//
// GCKCastContext è inizializzato in AppDelegate (inject_cast_appdelegate.py)
// così la discovery dei dispositivi parte all'avvio dell'app.
//
// JS usage (useChromecast.ts):
//   const NativeCast = registerPlugin('NativeCast');
//   await NativeCast.initialize({ appId: '6666EC62' });  // Fermenta custom receiver
