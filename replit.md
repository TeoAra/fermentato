...
- **Label Scanner**: Camera-based beer label/barcode scanner at `/scan`. Uses native BarcodeDetector API for EAN barcodes (lookup via Open Food Facts), falls back to PaddleOCR (Python, VPS) → Tesseract 5 → OCR.space Engine 2 cascade. PaddleOCR v3.4.0 + PaddlePaddle 3.0.0 installed on VPS with `FLAGS_use_mkldnn=0` to disable OneDNN/MKL-DNN which caused PIR attribute conversion crashes. Script at `/www/nodeapps/fermenta/server/paddle_ocr.py`.
