...
- **Pulizia dati ricerca (soft-archive birrifici/birre)**: archiviazione reversibile (mai delete) di birrifici chiusi e birre fuori produzione, escluse da ricerca, liste, suggerimenti **e contatori/statistiche**. Nessun LLM/GPU.
  ...
  - Cascata: archiviare un birrificio marca le sue birre `isDiscontinued` con `discontinuedSource='cascade'`; il ripristino riattiva solo quelle archiviate dalla cascata (non tocca quelle archiviate manualmente, `source='admin'`).
