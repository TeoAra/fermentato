...
- **Full Beer Import (Mar 2026)**: Imported 1,192,751 beers from RateBeer CSV export (2 files: A-J + K-Z, ~1.23M rows). Import script `scripts/import-beers-csv.ts` uses Neon WebSocket pool + `ON CONFLICT DO NOTHING` via partial unique index `idx_beers_name_brewery` on `(name, brewery_id)`. GIN trigram indexes added (`idx_beers_name_trgm`, `idx_beers_style_trgm`) for fast ILIKE search. `searchBeers()` query now has `LIMIT 50` — search response time: ~220ms (down from 5s).
